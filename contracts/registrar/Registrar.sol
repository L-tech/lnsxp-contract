...
contract Registrar is IRegistrar, ERC721Enumerable, Ownable {
    ...
    // The LNS registry
    LNS public lns;
    // The namehash of the TLD this registrar owns (eg, .link)
    ...
    constructor(LNS _lns, bytes32 _baseNode) ERC721("LNS", "LNS") {
        lns = _lns;
        baseNode = _baseNode;
    }
    ...
    function _register(
        ...
        _mint(_owner, id);
        if (updateRegistry) {
            lns.setSubnodeOwner(baseNode, bytes32(id), _owner);
            if (resolver != address(0)) {
                lns.setSubnodeResolver(baseNode, bytes32(id), resolver);
            }
        }
    ...
    /**
     * @dev Reclaim ownership of a name in LNS, if you own it in the registrar.
     */
