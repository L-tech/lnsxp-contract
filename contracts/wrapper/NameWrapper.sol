...
import {LNS} from "../registry/LNS.sol";
    ...
    ERC20Recoverable
{
     ...
     * @param id Label as a string of the .link domain to wrap
        ...
        onlyOwner
    {
        if (address(upgradeContract) != address(0)) {
            ...
            lns.setApprovalForAll(address(upgradeContract), false);
        }
        ...
        if (address(upgradeContract) != address(0)) {
            ...
            lns.setApprovalForAll(address(upgradeContract), true);
        }
    }
    ...
    /**
     * @notice Wraps a .link domain, creating a new token and sending the original ERC721 token to this contract
     * @dev Can be called by the owner of the name on the .link registrar or an authorised caller on the registrar
     * @param label Label as a string of the .link domain to wrap
    ...
    function wrapETH2LD(
        ...
        // transfer the lns record back to the new owner (this contract)
    ...
    /**
     * @dev Registers a new .link second-level domain and wraps it.
     *      Only callable by authorised controllers.
     * @param label The label to register (Eg, 'foo' for 'foo.link').
     * @param wrappedOwner The owner of the wrapped name.
     * @param duration The duration, in seconds, to register the name for.
     * @param resolver The resolver address to set on the LNS registry (optional).
     * @param ownerControlledFuses Initial owner-controlled fuses to set
     * @return registrarExpiry The expiry date of the new name on the .link registrar, in seconds since the Unix epoch.
     */
    ...
    /**
     * @notice Renews a .link second-level domain.
     * @dev Only callable by authorised controllers.
     * @param tokenId The hash of the label to register (eg, `keccak256('foo')`, for 'foo.link').
     * @param duration The number of seconds to renew the name for.
     * @return expires The expiry date of the name on the .link registrar, in seconds since the Unix epoch.
     */
    ...
    /**
     * @notice Wraps a non .link domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
    ...
    function wrap(
        ...
        if (owner != msg.sender && !lns.isApprovedForAll(owner, msg.sender)) {
            revert Unauthorised(node, msg.sender);
        }

        if (resolver != address(0)) {
            lns.setResolver(node, resolver);
        }
    ...
    /**
     * @notice Unwraps a .link domain. e.g. vitalik.link
     * @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
     * @param labelhash Labelhash of the .link domain
     * @param registrant Sets the owner in the .link registrar to this address
    ...
    /**
     * @notice Unwraps a non .link domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
     * @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
    ...
    /**
     * @notice Upgrades a .link wrapped domain by calling the wrapETH2LD function of the upgradeContract
     *     and burning the token of this contract
     * @dev Can be called by the owner of the name in this contract
     * @param label Label as a string of the .link name to upgrade
     * @param wrappedOwner The owner of the wrapped name
     */
    ...
    /**
     * @notice Upgrades a non .link domain of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
    ...
    /** 
    /* @notice Sets fuses of a name that you own the parent of. Can also be called by the owner of a .link name
     * @param parentNode Parent namehash of the name e.g. vitalik.xyz would be namehash('xyz')
     * @param labelhash Labelhash of the name, e.g. vitalik.xyz would be keccak256('vitalik')
    ...
    function setChildFuses(
        ...
        if (owner == address(0) || lns.owner(node) != address(this)) {
    ...
    function setSubnodeOwner(
        ...
        if (!isWrapped(node)) {
            lns.setSubnodeOwner(parentNode, labelhash, address(this));
    ...
    /**
     * @notice Sets records for the name in the LNS Registry
    ...
    {
        lns.setRecord(node, address(this), resolver, ttl);
        if (owner == address(0)) {
    ...
    function _preTransferCheck(
        ...
        // For this check, treat .link 2LDs as expiring at the start of the grace period.
        if (fuses & IS_DOT_ETH == IS_DOT_ETH) {
            expiry -= GRACE_PERIOD;
        }
    ...
    function _normaliseExpiry(
        ...
        // .link names will check registrar, non .link check parent
        if (expiry > maxExpiry) {
            expiry = maxExpiry;
        }
